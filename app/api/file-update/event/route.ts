...
export async function POST(req: NextRequest,) {
    ...
    try {
        ...
        // 1. Check if the images limit is reached
        const eventimagetypecount = await db.$count(eventImages, and(eq(eventImages.eventId, event.id), eq(eventImages.imageType, ImageType!)))
        if (!ImageType) {
            return NextResponse.json({ success: false, message: "Image type is missing" }, { status: 400 });
        }

        if (ImageType === 'thumbnail' && eventimagetypecount >= 1) {
        ...
        const result = await new Promise<CloudinaryUploadResponse>((resolve, reject) => {
            const uploadStream = cloudinary.uploader.upload_stream(
                { folder: `event/${Id}`, resource_type: "auto" },
                (error, result) => {
                    if (error) {
                        reject(error);
                    } else {
        ...
        return NextResponse.json({ success: true, data: newEventImage, message: "Uploaded File Successfully", }, { status: 200 });
